...
{ public Control() { controls = new ControlCollection(this); }
  ...
  public int BorderWidth { get { return UIHelpers.BorderWidth(border); } }
  ...
  { if(PaintBackground!=null) PaintBackground(this, e);
    ...
    { Point at = UIHelpers.CalculateAlignment(PaddingRect, new Size(backImage.Width, backImage.Height), backImageAlign);
      backImage.Blit(e.Surface, at.X, at.Y);
    }
    if(border!=BorderStyle.None && (dontDraw&DontDraw.Border)==0)
      UIHelpers.DrawBorder(e.Surface, DrawRect, border, BorderColor);
  }
  ...
  { if(lockParent) return; // if we're manipulating the ControlCollection and don't want it changing our Parent, just return
  ...
  bool enabled=true, visible=true, mychange, keyPreview, pendingPaint, pendingLayout, lockParent;
}
#endregion
